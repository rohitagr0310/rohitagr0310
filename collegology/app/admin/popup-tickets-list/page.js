...
export default function StudentTickets() {
  ...
  useEffect(() => {
    // Fetch all leads
    fetch(`${API.API_BASE_URL}/api/popup-tickets/get-all`)
  ...
  const filteredLeads = leadList.filter((lead) =>
    lead.name.toLowerCase().startsWith(search.toLowerCase())
  );

  const openPopup = (lead) => {
  ...
  const assignConsultant = (consultantId) => {
    fetch(`${API.API_BASE_URL}/api/popup-tickets/update-consultant`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json"
      },
      body: JSON.stringify({ ticketId: selectedLead._id, consultantId })
    })
      .then((response) => response.json())
      .then((updatedLead) => {
        setLeadList((prevLeads) =>
          prevLeads.map((lead) =>
            lead._id === selectedLead._id
              ? {
                  ...lead,
                  assigned_consultant: updatedLead.ticket.assigned_consultant
                }
              : lead
          )
        );
    ...
    <div className="p-4 bg-gray-100 min-h-screen flex flex-col items-center w-full">
      ...
      <div className="w-full max-w-6xl bg-white p-4 rounded shadow-md overflow-x-auto md:w-11/12 lg:w-10/12 xl:w-8/12">
        <h2 className="text-xl font-semibold mb-4 text-gray-700 flex items-center gap-2">
          <User className="text-gray-700" size={24} /> Leads
        </h2>
